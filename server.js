...
// ===== SQLite persistence (1-service) =====
// Tip: On Render, add a Persistent Disk mounted at /var/data so ranking/history won't reset on redeploy.
...
// REST API for ranking/history
app.get("/api/health", (_req,res)=>res.json({ok:true, db: DB_PATH}));
app.get("/api/ranking", async (req,res)=>{
  try{
    const limit = Math.min(200, Math.max(1, parseInt(req.query.limit||"50",10)));
    const items = await dbAll(`SELECT display_name, rating, wins, losses, games FROM players WHERE games>0 ORDER BY rating DESC, wins DESC, games DESC LIMIT ?`, [limit]);
    res.json({items});
  }catch(e){
...
app.get("/api/history", async (req,res)=>{
  try{
    ...
    const items = await dbAll(`SELECT mr.ended_at, m.room_code, m.mode, mr.placement, mr.exploded, mr.delta FROM match_results mr JOIN matches m ON m.id = mr.match_id WHERE mr.player_token=? ORDER BY mr.ended_at DESC, mr.id DESC LIMIT ?`, [tok, limit]);
    res.json({items});
  }catch(e){
...
// List rooms for lobby (show rooms that are waiting / not started)
app.get("/api/rooms", (req,res)=>{
  try{
    const items = Array.from(rooms.values())
      .filter(r=>!r.started && r.players.length>0 && r.players.length<10)
    ...
    res.status(500).json({error:"rooms failed", detail:String(e)});
  }
});
